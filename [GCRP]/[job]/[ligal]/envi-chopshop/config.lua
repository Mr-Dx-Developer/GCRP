...
-- [NEW] --
Config.InteractionKey = 38 -- Change the key for interactions for Start Chop Vehicle and Manage Chop Shop
...
-- [THIS WILL REQUIRE YOU TO CODE YOUR OWN LOGIC IN THE SERVER_EDIT.LUA FILE FOR CHECKING OWNERSHIP AND IF YOU WANT TO DELETE, OR RETURN THE VEHICLE TO THE IMPOUND ETC ]
...
function SoundEvent(coords, soundFile, volume) -- Change this to your own sound system if you want
    TriggerServerEvent('InteractSound:PlayFromCoord', coords, 25, soundFile, volume)
end
...
Config.Tools = {
    ...
    SawProp = { prop = `v_ind_cs_powersaw`, offsets = {0.22, 0.34, 0.03, -120.64, -158.35, -64.21}}  -- Saw Prop - (must be in backticks like `prop_name`)
}
...
function DispatchEvent(coords) -- Change this to your own dispatch system if you want

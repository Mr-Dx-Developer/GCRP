...
-- Note: rcore_prison [API]
-- This code was added by rcore_prison to support booths and calls for qb-phone

HandleCallState = function(ContactData, cb)
	local phoneNumber = ContactData.number
	local boothData = GlobalState.Booths and GlobalState.Booths[tonumber(phoneNumber)] or nil
	...
	if boothData then
		...
		target = QBCore.Functions.GetPlayerByPhone(phoneNumber)
	end

	if target then
		if targetType == 'call' then
...
-- This part was modified by rcore_prison to add support for booths.

QBCore.Functions.CreateCallback('qb-phone:server:GetCallState', function(_, cb, ContactData)
	HandleCallState(ContactData, cb)
end)
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    TriggerEvent('qs-smartphone:server:sendNewMailToOffline', sendercitizenid, invoiceMailData)
...
RegisterNetEvent('qs-smartphone:server:sendNewMail', function(mailData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qs-smartphone:server:sendNewMailToOffline', function(citizenid, mailData)

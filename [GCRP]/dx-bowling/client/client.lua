    ...
    while true do
        ...
        if not bowling then
            ...
            if distance <= 15.0 then
                sleep = 0
                DrawMarker(2, Config['Bowling']['GetBall'], vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(0.25, 0.25, 0.2), 255, 255, 255, 255, true)
                if distance <= 1.5 then
                    helpText(Strings['press_start'])
                    if IsControlJustReleased(0, 38) then
                        bowling = true
                        TriggerServerEvent('dx-bowling:start')
                    end
                end
            end
        end
...
RegisterNetEvent('dx-bowling:cancel')
AddEventHandler('dx-bowling:cancel', function(full)
    if full then
...
RegisterNetEvent('dx-bowling:failed')
AddEventHandler('dx-bowling:failed', function()
    bowling = false
end)


RegisterNetEvent('dx-bowling:chooseTrack')
AddEventHandler('dx-bowling:chooseTrack', function(occupiedTracks)
    ...
    if #tracksLeft >= 1 then
        ...
        while true do
            ...
            DrawMarker(22, current.x, current.y, current.z+1.0, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(0.5, 0.5, 0.4), 35, 255, 75, 255)
            ...
            if IsControlJustReleased(0, 215) then
                for i = 1, #Config['Bowling']['Tracks'] do
                    if Config['Bowling']['Tracks'][i] == tracksLeft[currentTrack] then
                        TriggerServerEvent('dx-bowling:choose', i)
                    end
                end
                break
...
RegisterNetEvent('dx-bowling:play')
AddEventHandler('dx-bowling:play', function(chosenTrack)
    bowling = true 
    DoScreenFadeOut(750)
    ...
    DestroyCam(cam, false)
    ClearPedTasks(me)
    TriggerServerEvent('dx-bowling:end', chosenTrack)
    for k, v in pairs(pins) do DeleteEntity(v) end
    bowling = false
end)

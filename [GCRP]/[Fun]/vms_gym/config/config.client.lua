...
Citizen.CreateThread(function()
    while true do
        ...
        elseif DoesEntityExist(myVehicle) then
            ...
            if GetVehicleClass(myVehicle) == 13 and speed >= Config.AddStatsValues['Cycling'].minimumSpeed and Config.StatisticsMenu['condition'] then
            ...
            if speed >= Config.AddStatsValues['Driving'].minimumSpeed and Config.StatisticsMenu['driving'] then
        ...
        if myStatistics then
            if myStatistics['strenght'] and Config.EnableStrenghtModifier then
                if myStatistics['strenght'] >= 70.0 then
                    SetWeaponDamageModifier(GetHashKey("WEAPON_UNARMED"), 1.1)
                ...
                elseif myStatistics['strenght'] >= 20.0 then
                    ...
                    SetWeaponDamageModifier(GetHashKey("WEAPON_UNARMED"), 0.8)
                end
            end
            if myStatistics['condition'] then
                if myStatistics['condition'] >= 90.0 then
                     if Config.EnableRunSpeedModifier then
                        SetRunSprintMultiplierForPlayer(PlayerId(), 1.10)
                    end
                    if Config.EnableStaminaModifier then
                       StatSetInt(GetHashKey('MP0_STAMINA'), 80, true)
                    end
                    elseif myStatistics['condition'] >= 70.0 then
                    if Config.EnableRunSpeedModifier then
        ...
        if myVeh and GetPedInVehicleSeat(myVeh, -1) == PlayerPedId() then
            if (GetEntitySpeed(myVeh) > 30.0) then
                ...
                if myDrivingSkill < 20.0 then
                    local biasRandom = (math.random(-1.2, 1.2) + 0.0)
                ...
                elseif myDrivingSkill >= 20.0 and myDrivingSkill < 50.0 then
                    local biasRandom = (math.random(-1.2, 1.2) + 0.0) / 10
                    SetVehicleSteerBias(myVeh, biasRandom)

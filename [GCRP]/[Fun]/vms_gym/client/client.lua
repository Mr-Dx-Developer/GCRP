...
Citizen.CreateThread(function()
    for k, v in pairs(Config.Gyms) do
        if v.blipson then
            ...
            SetBlipSprite(v._createdBlip, v.blips['Sprite'])
            SetBlipDisplay(v._createdBlip, v.blips['Display'])
            SetBlipScale(v._createdBlip, v.blips['Scale'])
            SetBlipColour(v._createdBlip, v.blips['Color'])
                    ...
                    if not crashedSkillbar then
                        ...
                        if _pointTable.addSkill and _pointTable.addSkill.skill and _pointTable.addSkill.value then
                            if type(_pointTable.addSkill.value) == 'number' then
                                ...
                                addSkill(_pointTable.addSkill.skill, math.random(_pointTable.addSkill.value[1], _pointTable.addSkill.value[2])/10)
                            end
                        end
                    end
                else
...
RegisterNetEvent('vms_gym:notification', function(title, message, time, icon, type)

...
if IsDuplicityVersion() then
	...
	AddEventHandler('rcore_prison:prisonerState', function(data)
		...
		if state == 'released' then
			...
			dbg.debug('Prisoner [%s] was released from Prison with SID [%s]', data.userName, data.serverId)
	...
	CreateThread(function()
		...
		if Prison.CS.ModuleState then
			if Prison.CS.Blip.enable then
				table.insert(shared.data.interaction, {
					blip = {
						state = Prison.CS.ModuleState,
					...
					coords = Prison.CS.Place,
					type = 'community_service',
					name = l('COMMUNITY_SERVICE_LABEL'),
				})
			end
		end

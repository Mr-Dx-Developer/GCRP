...
---==[[ SAVE EXTRA DAMAGES ]]==---
	...
	if Config.vehFailure.repairKits then
		createUseableItem("repairkit", function(source, item)
			TriggerClientEvent("jim-mechanic:vehFailure:RepairVehicle", source, item, false)
		end)

		createUseableItem("advancedrepairkit", function(source, item)
			TriggerClientEvent("jim-mechanic:vehFailure:RepairVehicle", source, item, true)
		end)
	end
...
if GetResourceState(OXLibExport):find("start") then
	...
	createCallback("jim-mechanic:checkCash", function(source) local src = source
	...
	createCallback('jim-mechanic:mechCheck', function(source)
		local result = false
		local dutyList = {}
	...
	createCallback('jim-mechanic:GetXenonColour', function(source) return xenonColour end)
	createCallback('jim-mechanic:GetNosLoaded', function(source) return VehicleNitrous end)
	createCallback('jim-mechanic:GetNosColour', function(source) return nosColour end)
...
elseif GetResourceState(QBExport):find("start") or GetResourceState(ESXExport):find("start") then
	...
	createCallback('jim-mechanic:mechCheck', function(source, cb)
		...
		for _, playerId in ipairs(GetPlayers()) do
			...
			for _, v in pairs(Config.Main.JobRoles) do
				if Player.job.name == v and (Player.job.onduty and Player.job.onduty or true) then
					dutyList[tostring(v)] = true
				end
			end
		end
	...
	createCallback('jim-mechanic:GetXenonColour', function(source, cb) cb(xenonColour) end)

	createCallback('jim-mechanic:GetNosLoaded', function(source, cb) cb(VehicleNitrous) end)
	createCallback('jim-mechanic:GetNosColour', function(source, cb) cb(nosColour) end)

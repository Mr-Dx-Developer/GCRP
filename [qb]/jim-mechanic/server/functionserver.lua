...
---==[[ SAVE EXTRA DAMAGES ]]==---
	RegisterNetEvent("jim-mechanic:updateVehicle", function(props, plate)
		if IsVehicleOwned(props.plate) then
			if Config.System.Debug then print("^5Debug^7: ^3updateVehicle^7: ^2Vehicle Mods^7 - [^6"..plate.."^7]: ^4"..json.encode(props).."^7") end
			MySQL.Async.execute('UPDATE player_vehicles SET mods = ? WHERE plate = ? AND hash = ?', { json.encode(props), plate, props.model})
		end
	end)

local QBCore = exports['qb-core']:GetCoreObject()
ItemsPlayerCanBuy = { -- Prevents Exploiting...
  "combomeal",
  "bigmac",
  "mcpollo",
  "mcroyaldeluxe",
  "cbo",
  "cuartodelibra",
  "grandmcextreme",
  "bigchickensupreme",
  "bigcrispybbq",
  "bigdoublecheese",
  "hamburgesa",
  "hamburgesadepollo",
  "mcfish",
  "happymeal",
  "cocacola",
  "nestea",
  "agua",
  "monsterenergy",
  "cerveza",
  "aquarius",
  "sprite",
  "colacao",
  "nuggets",
  "patatasfritas",
  "topfries",
  "fanta",
}

RegisterServerEvent('vb-mcdonalds:realizarpedido')
AddEventHandler('vb-mcdonalds:realizarpedido', function(data)
    local _src = source
    local _char = QBCore.Functions.GetPlayer(_src)
    local _charmoney = _char.PlayerData.money["cash"]
    local _price = tonumber(data.price * data.amount)
    local _isitemvalid = false
    for k,v in pairs(ItemsPlayerCanBuy) do
      if v == data.item then
        _isitemvalid = true
      end
    end
    if _isitemvalid then
      if _charmoney >= _price and _price > 0 then
        _char.Functions.RemoveMoney('cash',_price)
        _char.Functions.AddItem(data.item, data.amount)
        TriggerClientEvent('inventory:client:ItemBox', _src, QBCore.Shared.Items[data.item], "add")
        exports['qb-management']:AddMoney("sheriff", _price)
        TriggerClientEvent('QBCore:Notify', _src, "You Bought One RM " .._price, 'success')
        
      else
        TriggerClientEvent('QBCore:Notify', _src, "You Dont Have enough Cash RM " .._price, 'success')
      end
    else
      DropPlayer(_src, "Player has triggered a VB-McDonald's event to purchase an item that isn't on the valid item list!")
    end
end)
